...
namespace ProjetoDDD.MVC.App_Start
{
    ...
    public static class NinjectWebCommon 
    {
        ...
        private static void RegisterServices(IKernel kernel)
        {
            ...
            kernel.Bind<ICategoriaDoProdutoAppService>().To<CategoriaDoProdutoAppService>();

            kernel.Bind(typeof(IServiceBase<>)).To(typeof(ServiceBase<>));
            kernel.Bind<IClienteService>().To<ClienteService>();
            kernel.Bind<IProdutoService>().To<ProdutoService>();
            kernel.Bind<ICategoriaDoProdutoService>().To<CategoriaDoProdutoService>();

            kernel.Bind(typeof(IRepositoryBase<>)).To(typeof(RepositoryBase<>));
            kernel.Bind<IClienteRepository>().To<ClienteRepository>();
            kernel.Bind<IProdutoRepository>().To<ProdutoRepository>();
            kernel.Bind<ICategoriaDoProdutoRepository>().To<CategoriaDoProdutoRepository>();
        }
    }
}
